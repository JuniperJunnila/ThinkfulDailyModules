...
var validateSearch = function validateSearch(formData) {
  var validateExists = function validateExists(value) {
    return value && value.trim();
  };

  if (!validateExists(formData.get("searchTerm"))) return "Please enter a search term";
};

var submitHandler = function submitHandler(event) {
  event.preventDefault();
  var searchData = new FormData(event.target);
  var searchTerm = searchData.get("searchTerm");
  ...
  var lowerTitles = Array.from(titles).map(function (title) {
    return title.innerText.toLowerCase();
  });
  var hiddenList = document.querySelectorAll('.hidden');

  if (hiddenList.length) {
    var _iterator = _createForOfIteratorHelper(hiddenList),
        _step;
  ...
  lowerTitles.map(function (title) {
    if (!title.includes(searchTerm.toLowerCase())) {
      var index = lowerTitles.indexOf(title);
